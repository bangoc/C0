...
struct part *realloc_inventory(int new_size) {
  struct part *tmp = realloc(inventory, new_size * sizeof(struct part));
  if (tmp) {
...
void insert(void)
{
  ...
  if (num_parts == inventory_size) {
    if (realloc_inventory(inventory_size * 2) == NULL) {
      printf("CSDL đã đầy. Không thể lưu thêm linh kiện mới.\n");
      return;
    }
  }
