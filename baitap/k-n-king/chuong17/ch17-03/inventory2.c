...
/**********************************************************
 * erase: Hỏi người dùng nhập mã linh kiện. Sau đó xóa    *
 *        linh kiện người dùng đã nhập vào.               *
 **********************************************************/
void erase(void)
{
  int number;
  printf("Nhập mã linh kiện: ");
  scanf("%d", &number);
  struct part *p = inventory, *prev = NULL;
  while (p != NULL && p->number != number) {
    prev = p;
    p = p-> next;
  }
  if (p == NULL) {
    printf("Không tìm thấy linh kiện với mã số %d\n", number);
    return;
  }
  if (p == inventory) {
